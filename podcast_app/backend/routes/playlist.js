...
router.put("/add-song", auth, async (req, res) => {
	...
	if (playlist.podcast.indexOf(req.body.songId) === -1) {
		playlist.podcast.push(req.body.songId);
	}
	await playlist.save();
...
router.put("/remove-song", auth, async (req, res) => {
	...
	if (!user._id.equals(playlist.user))
		return res
			.status(403)
			.send({ message: "No access to remove" });

	const index = playlist.podcast.indexOf(req.body.songId);
	playlist.podcast.splice(index, 1);
	await playlist.save();
...
router.get("/:id", [validateObjectId, auth], async (req, res) => {
	...
	const podcast = await Song.find({ _id: playlist.podcast });
	res.status(200).send({ data: { playlist, podcast } });
});
